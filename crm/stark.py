...
class ClassConfig(ModelStark):
    ...
    list_display = [display_classname, "tutor", "teacher"]
...
class CustomerConfig(ModelStark):
    ...
    def display_gender(self, obj=None, header=False):
        if header:
            return "性别"
        return obj.get_gender_display()  # 固定语法 get_字段_display() 展示数据对应的值  choice
    ...
    def further(self, request, customer_id):
        ...
        user_id = request.session.get("user_id")  # alex
        now = datetime.datetime.now()
        delta_day3 = datetime.timedelta(days=3)
        delta_day15 = datetime.timedelta(days=15)
        # 为改客户更改课程顾问，和对应时间
    ...
    def extra_url(self):
        ...
        temp.append(path(r"public/", self.public_customer))
        temp.append(re_path(r"further/(\d+)", self.further))
        temp.append(path(r"mycustomer/", self.mycustomer))
        return temp
...
class CourseRecordConfig(ModelStark):
    ...
    def patch_studyrecord(self, request, queryset):
        ...
        for course_record in queryset:
            stu_list = Student.objects.filter(class_list__id=course_record.class_obj.pk)
